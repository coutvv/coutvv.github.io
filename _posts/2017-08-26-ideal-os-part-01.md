...
TL;DR: Данной статьёй Я хочу показать что:
...
Или другой пример. Atom -- один из самых популярных текстовых редакторов на сегодня. Разработчики обожают его потому что к нему есть огромное количество плагинов, но давайте разберёмся каким образом он работает. Atom использует Electron, который по существу является полноценным веб-браузером, обручённый с окружением NodeJS. Это целых два Javascript-движка объединённых в единое приложение. Приложения на Electron используют API отрисовки браузера, которые делегируют отрисовку нативным библиотекам, которые затем обращаются к GPU(если повезёт) для всамделешней отрисовке. Так много слоёв..
.пщщп
...
CMD.exe, консольная программа позволяющая запускать программы DOS, была заменена только в 2016 году. А самая большая фича последнего релиза Windows 10 состоит в том, что они добавили подсистему Linux. Всё слоёнее и слоёнее пирог выходит.

## X Windows

![image](https://joshondesign.com//images2/compiz.jpg)

X Windows улучшалась даже меньше чем два других десктопных окружения. На самом деле, это примечательный пример неизменяемости. Люди жалуются на это с начала 90ых. Я очень рад, что могу приодеть свой GUI, но как насчёт системного буфера обмена, в котором нельзя хранить более одного элемента за раз? Это неизменно с 80ых! 
...
Wayland предположительно должен был всё исправить, но он разрабатывается уже десятилетие и всё ещё не готов! Совместимость со старыми API действительно тяжела. Мне кажется Apple была права, когда засунула старый macOS в эмулятор, отгородившись от старого кода. 
...
Почему компьютер не использует ничего для вывода кроме дисплея? Мой новый Razer-ноутбук имеет RGB-подсветку под каждой клавишей и единственная их функциональность -- переливание цветными волнами! Как насчёт использовать эти LED для чего-то [реально полезного](https://arcan-fe.com/2017/05/21/playing-with-leds/)?
...
Но самое важное, что слабая коммуникация между приложениями делает очень сложным их координацию. Фундаментальные принципы Unix состоят в том, что небольшие инструменты работают вместе. Но X Windows не поддерживает это от слова совсем
...
Проблема в том, что на дворе 2017. Мы уже не работаем тем способом, что и в 1984. В типичный день Я возьму некий код с самых разных удалённых сайтов, скомпоную несколько тестов и сгенерирую структуру данных, представляющую результат, затем отправлю полученное другим для переиспользования. Импорт, синтез, экспорт. 
